...
void power_on_screen(void)
{
    ...
    clcd_print("Microwave Oven",(0x90 + 1));
    for(int i = 0;i < 16;i++){
...
void set_time(unsigned char key){
    ...
    if(wait++ == 50){
        wait = 0;
        blink = !blink;
        clcd_print(min_arr,(0xC0 + 6));

        clcd_print(sec_arr,(0xC0 + 9));
    }
    if(blink){
        switch(blink_posi){
            case 0:
...
void door_status_check(void)
{
    ...
    if(RB3 == 0)
    {
        ...
        TMR2ON = 0;
        clear_display();
        clcd_print("Door Open",(0xC0 + 2));
        clcd_print("Please close",(0x90 + 2));
        while(RB3 == 0)
        {
            ;
        }
...
void time_display(void)
{
    ...
    if(min == 0 && sec == 0)
    {
        ...
        RC2 = 0;
        clear_display();
        clcd_print("COOKING TIME UP",(0x80 + 0));
